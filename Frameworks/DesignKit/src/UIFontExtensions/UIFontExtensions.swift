...
public extension UIFont {
    static let designKit = DesignKitFont.self

    @available(iOS 13.0, *)
    enum DesignKitFont {
        public var display1: UIFont {
            scaled(baseFont: .systemFont(ofSize: 42, weight: .semibold), forTextStyle: .largeTitle, maxFactor: 1.5)
        }

        public var display2: UIFont {
            scaled(baseFont: .systemFont(ofSize: 36, weight: .semibold), forTextStyle: .largeTitle, maxFactor: 1.5)
        }
...
@available(iOS 13.0, *)
private extension UIFont.DesignKitFont {
    @available(iOS 13.0, *)
    func scaled(baseFont: UIFont, forTextStyle textStyle: UIFont.TextStyle = .body, maxFactor: CGFloat? = nil) -> UIFont {
        let fontMetrics = UIFontMetrics(forTextStyle: textStyle)

        if let maxFactor = maxFactor {
            let maxPointSize = baseFont.pointSize * maxFactor
            return fontMetrics.scaledFont(for: baseFont, maximumPointSize: maxPointSize)
        }
